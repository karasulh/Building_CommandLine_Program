...
fn main() {
    ...
    //args are given arguments when trying to run program by "cargo run pair poem.txt" 
    //(btw:args[0]=target\debug\minigrep.exe,args[1]=pair,args[2]=poem.txt)

    //Config::new is a method to parse arguments of args
    let config=Config::new(env::args()).unwrap_or_else(|err|{//env::args returns an iterator //use closure to handle error
